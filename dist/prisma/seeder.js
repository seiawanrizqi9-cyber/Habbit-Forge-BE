...
async function main() {
    ...
    const johnHealthHabit = habits.find((h) => h.userId === john.id && h.title.includes("Minum Air"));
    const johnLearningHabit = habits.find((h) => h.userId === john.id && h.title.includes("Baca Buku"));
    const janeHealthHabit = habits.find((h) => h.userId === jane.id && h.title.includes("Meditasi"));
    if (johnHealthHabit) {
